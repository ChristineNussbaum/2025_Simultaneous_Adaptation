...
# outcome: Resp
# predictors: ML, Word, AdaptType #we removeSpSex
...
#-------------------------------------------------------------------------------#
#   Analysis 1b: try the same with brms for estimation of the Baysian effect    #
#-------------------------------------------------------------------------------#


##get derault priors first
default_prior <- default_prior(Resp ~ tML_sc + AdaptType + Word + tML_sc:Word  + tML_sc:AdaptType + Word:AdaptType
                               + (1|SpID) + (1 | Participant),
                        data = E3_Adapt,  family = bernoulli(link = "logit"))

#Full Model
# I took SpSex out, to not complicate the model tooo much
fullmodel <- brm(Resp ~ tML_sc + AdaptType + Word + tML_sc:Word  + tML_sc:AdaptType + Word:AdaptType
                + (1|SpID) + (1 | Participant),
                         data = E3_Adapt,
...
default_prior <- default_prior(Resp ~ tML_sc + AdaptType + Word + tML_sc:Word  + tML_sc:AdaptType 
                               + (1|SpID) + (1 | Participant),
                               data = E3_Adapt,  family = bernoulli(link = "logit"))
...
bayes_factor(redmodel, fullmodel)
...
#aggregate across pseudoword, AdaptType and SpSex for analysis
...
a<-ezANOVA(data=E3_Resp, dv=.(Resp), wid=.(Participant), within = .(AdaptType, Word, SpSex), type=3, detailed = TRUE)
...
#convert them into wide format for JASP
E3_wide <- pivot_wider(E3_Resp[,1:5], names_from = c(AdaptType, Word, SpSex), values_from = c(Resp))

#save this dataset for JASP Analysis
write.csv(E3_wide, file = "JASP/pw_analysis_anova2.csv")
